...
#define NUM_VIDEO_SURFACES_WMV3   3 /* 1 decode frame, up to  2 references */
...
static int                      g_last_config_result;
...
static int VAProfile_from_imgfmt(uint32_t format)
{
    ...
    static const int h263_profiles[] =
        { VAProfileH263Baseline, -1 };
#if VA_CHECK_VERSION(0,37,0)
...
static int config_vaapi(uint32_t width, uint32_t height, uint32_t format)
{
    ...
    if (!IMGFMT_IS_VAAPI(format))
        ...
        case IMGFMT_VAAPI_VC1:
            ...
            break;
        case IMGFMT_VAAPI_H263:
            num_surfaces = NUM_VIDEO_SURFACES_H263;
            break;
        case IMGFMT_VAAPI_HEVC:
...
// HACK - this fills in the ffmpeg hwaccel context held in vd_ffmpeg.c.
void vo_vaapi_fill_hwaccel_context(struct vaapi_context *hwaccel)
{
    assert(va_context);
    memcpy(hwaccel, va_context, sizeof(struct vaapi_context));
}
                  ...
                  uint32_t flags, char *title, uint32_t format)
{
    ...
    if (width  == g_image_width  &&
        ...
        mp_msg(MSGT_VO, MSGL_DBG2, "[vo_vaapi] config(): same as last time, returning %d\n", g_last_config_result);
